...
uint32_t transfer(char **arguments, assembler *instState,table_t *t){
 ...
 if(address[0] == '[' && address[4] != ']' && address[3] != ']'){
  ...
  printf("INCVAL:%s\n",arguments[1]);
  uint32_t incVal = strtol(removeBrackets(arguments[2]),NULL,10);
  //1110 01 1 1 1 00 1 rn rd getConst(expression[1]);
  uint32_t offset = (incVal+rn)-1; //TODO: ONCE MOV is fixed replace this with the val generated by mov
